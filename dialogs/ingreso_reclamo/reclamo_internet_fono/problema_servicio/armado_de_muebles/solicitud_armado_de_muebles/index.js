...
bot.dialog('/solicitud_armado_de_muebles', [
  ...
  async (session, results, next) => {
    ...
    if (getOrder.success && getPaymentMethods.success && getPaymentMethods.state.type) {
      ...
      //session.userData.mediopago = getPaymentMethods.state.card_type.replace(':', '')
  ...
  async (session, results, next) => {
    if (!results.resumed) {
      if (results.response.entity == 'SI') {
        ...
        let msg = 'Si no has realizado la compra del servicio de armado, puedes dirigirte a cualquier de nuestras tiendas con la boleta del mueble que deseas armar y realizar la compra del servicio'
        session.beginDialog('/end_conversation', { mensaje: msg })
        // session.send()
  ...
  async (session, results, next) => {
    if (!results.resumed) {
      if (results.response.entity == 'SI') {
        session.beginDialog('/choiceMueble')
      } else {
        let msg = 'Si deseas solicitar el armado del producto en otra dirección puedes contactarte con nuestros ejecutivos al [600 380 5000](6003805000)'
        session.beginDialog('/end_conversation', { mensaje: msg })
        // session.send()
        // MensajeDeAyuda(session)
        // session.endConversation()
